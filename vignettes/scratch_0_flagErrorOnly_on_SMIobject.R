...
library(ggplot2)
library(FastReseg)
setwd("~/hades/dmcguire/smi/data_correctness/de")
devtools::load_all("../../FastReseg")

#main_output_dir <- '/home/rstudio/NAS_data/lwu/testRun/SpatialTest/giotto_test/melanoma/Run4104_melanoma_980plx/giotto_output'
main_output_dir <- '/home/rstudio/hades/dmcguire/data_correctness/de'
...
refClus_coln <- "cell_type" #nb_clus"
...
#source(fs::path(scAnalysis_dir, 'config_980plex.R'))
#source("~/smiqumulo/RnD/08. Mock TAP/02. NSCLC_1K/6.90 Analysis showcase full set/config_v1.2.R")
source("~/smiqumulo/RnD/08. Mock TAP/02. NSCLC_1K/6.92 Analysis showcase full set - hades backup/config_v1.2.R")
...
path_to_SMIobject <- fs::path("~/smiqumulo/RnD/08. Mock TAP/02. NSCLC_1K/5.2 flat files/Processed Data Giotto Object/SMI_Giotto_Object.RData")
...
debugonce(prepSMI_for_fastReseg)
...
colMeans(counts[metainfo[cell_type=="macrophage",cell_ID],] / Matrix::rowSums(counts[metainfo[cell_type=="macrophage",cell_ID],]))[1:3]
smi_inputs$refProfiles[1:3,]
...
saveRDS(reseg_outputs, file="fastreseg_reseg_outputs.rds") 

modstats <- data.table(reseg_outputs$combined_modStats_ToFlagCells)
metainfo <- copy(gem@cell_metadata$rna)
chk <- merge(metainfo[,.(cell_ID, cell_type,niche)], modstats, by.x="cell_ID", by.y="UMI_cellID",all.y=TRUE, all.x=TRUE)
chk[is.na(flagged),.N,by=cell_type]

flagcells <- chk[flagged==TRUE][cell_type!=tLLRv2_maxCellType][cell_type=="macrophage"][,cell_ID]
chk[,mean(flagged,na.rm=TRUE),by=cell_type]

library(smiDE)

metainfo <- 
merge(metainfo, modstats[,.(cell_ID=UMI_cellID, tLLRv2_maxCellType, lrtest_Pr,transcript_num)]
      ,by="cell_ID",all.x=TRUE,sort=FALSE)

metainfo <- merge(metainfo, gem@spatial_locs$raw, by="cell_ID")
pre_de_obj <- pre_de(counts = gem@expression$rna$raw
                     ,normalized_data = gem@expression$rna$normalized
                     ,metadata = metainfo
                     ,ref_celltype = "macrophage"
...
genes <- grep("KRT|IGH", rownames(gem@expression$rna$raw),value=TRUE)
metainfo[,`:=`(
  flagged01=FALSE
  ,flagged1e3=FALSE
  ,flagged1e5=FALSE
)]
metainfo[lrtest_Pr < 1e-5,flagged1e5:=TRUE]
metainfo[lrtest_Pr < 1e-3,flagged1e3:=TRUE]
metainfo[lrtest_Pr < 0.01,flagged01:=TRUE]
metainfo[,lapply(.SD, mean),.SDcols=grep("flagged",names(metainfo))]

allmacro <- metainfo[cell_type=="macrophage",cell_ID]
macro01 <- metainfo[cell_type=="macrophage" & flagged01==FALSE,cell_ID]
macro1e3 <- metainfo[cell_type=="macrophage" & flagged1e3==FALSE,cell_ID]
macro1e5 <- metainfo[cell_type=="macrophage" & flagged1e5==FALSE,cell_ID]
macro01topct <- metainfo[cell_type=="macrophage" & flagged01==FALSE & tLLRv2_maxCellType=="macrophage",cell_ID]
macrotopct_nopfilter <- metainfo[cell_type=="macrophage" & tLLRv2_maxCellType=="macrophage",cell_ID]
de01 <- 
smi_de(assay_matrix = gem@expression$rna$raw[,macro01]
       ,metadata = metainfo[cell_ID %in% macro01]
       ,formula = ~RankNorm(otherct_expr) + tissue + niche + offset(log(totalcounts))
       ,neighborhood_counts = pre_de_obj$nblist
       ,groupVar = "niche"
       ,targets = grep("KRT",genes,value=TRUE)
       ,family = "nbinom2"
       ,nCores=4
       )

results(de01, "one.vs.rest", "niche")[[1]][p.value < 0.05/960]

de01topct <- 
smi_de(assay_matrix = gem@expression$rna$raw[,macro01topct]
       ,metadata = metainfo[cell_ID %in% macro01topct]
       ,formula = ~RankNorm(otherct_expr) + tissue + niche + offset(log(totalcounts))
       ,neighborhood_counts = pre_de_obj$nblist
       ,groupVar = "niche"
       ,targets = grep("KRT",genes,value=TRUE)
...
results(de01topct, "one.vs.rest", "niche")[[1]][p.value < 0.05/960]
results(de01topct, "pairwise", "niche")[[1]][p.value < 0.05/960]

detopct <- 
smi_de(assay_matrix = gem@expression$rna$raw[,macrotopct_nopfilter]
       ,metadata = metainfo[cell_ID %in% macrotopct_nopfilter]
       ,formula = ~RankNorm(otherct_expr) + tissue + niche + offset(log(totalcounts))
       ,neighborhood_counts = pre_de_obj$nblist
       ,groupVar = "niche"
       ,targets = grep("KRT",genes,value=TRUE)
...
results(detopct, "pairwise", "niche")[[1]][p.value < 0.05/960]
results(detopct, "one.vs.rest", "niche")[[1]][p.value < 0.05/960]
...
smi_de(assay_matrix = gem@expression$rna$raw[,macro01topct]
       ,metadata = metainfo[cell_ID %in% macro01topct]
       ,formula = ~ tissue + niche + offset(log(totalcounts))
       ,neighborhood_counts = pre_de_obj$nblist
       ,groupVar = "niche"
       ,targets = grep("KRT",genes,value=TRUE)
       ,family = "nbinom2"
...
results(de01topct_nonb, "one.vs.rest", "niche")[[1]][p.value < 0.05/960]
results(de01topct_nonb, "pairwise", "niche")[[1]][p.value < 0.05/960]



mean(Matrix::rowMeans(counts[flagcells,grep("KRT",colnames(counts))]))
mean(Matrix::rowMeans(counts[,grep("KRT",colnames(counts))]))
