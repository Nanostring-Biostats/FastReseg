...
#' @description supporting function for \code{runPreprocess}, \code{fastReseg_internalRef} and \code{fastReseg_flag_all_errors}, to load transcript data.frame of each FOV from file path
#' @param path_to_fov
...
prepSMI_for_fastReseg <- function(path_to_SMIobject, 
  ...
  if(all(c('Width','Height') %in% colnames(cell_metadata))){
    ...
    ## if not exist, one could use the automatic estimation capacity of `fastReseg_perFOV_full_process` function
